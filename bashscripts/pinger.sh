...
# Defining because not all linux versions have the seq command
...
then
	...
	DATE=`date`
	echo " " >> $OUTFILE
	echo "[$DATE] - SUBNET:$1" >> $OUTFILE
	echo "-------------------------------------------" >> $OUTFILE
	for IP in $NUMS;do
		ping -c $TIMEOUT $1.$IP 2>/dev/null | tail -5 |grep -i "64 bytes" | cut -d " " -f4 | tr -d ":" | tee -a "$OUTFILE" &
